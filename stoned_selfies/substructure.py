...
def substructure_preserver(substructure, mol):
    """Check if substructure is found in a target molecule.

    Return True: contains a substructure violation
    Return False: No substructure violation
    """
    if mol.HasSubstructMatch(Chem.MolFromSmarts(substructure)) is True:
        return True  # The has substructure!
    else:
        return False  # Molecule does not have substructure!


class StonedChemicalSubspaceSubstructure:
    ...
    def __init__(
        ...
        """Implement the STONED algorithm to generate a local chemical subspace.
    ...
    def generate(self) -> List[str]:
        ...
        if isinstance(self.num_mutations, int):
            mutated_selfies = self._get_mutated_SELFIES(
                rand_seed_selfies, self.num_mutations, substructure=self.substructure
            )

        elif isinstance(self.num_mutations, list):
            for n in self.num_mutations:
                mutated_selfies = self._get_mutated_SELFIES(
                    rand_seed_selfies, num_mutations=n, substructure=self.substructure
                )
        else:
    ...
    def _get_mutated_SELFIES(
        self, selfies_ls: List[str], num_mutations: int, substructure: Mol
    ) -> List[str]:
        ...
        for _ in range(num_mutations):
            ...
            for str_ in selfies_ls:
                ...
                selfie_mutated, _ = self._mutate_selfie_substructure(
                    str_, substructure, max_molecules_len
                )
    ...
    def _get_randomize_smiles(self) -> List[str]:
        """Return a random (de-aromatized) SMILES given an rdkit mol object of a molecule.
    ...
    def _mutate_selfie_substructure(
        self,
        selfie: str,
        substructure: str,
        max_molecules_len: int,
        write_fail_cases: bool = False,
    ) -> Tuple[str, str]:
        ...
        write_fail_cases : bool, optional
            If true, failed mutations are recorded in "selfie_failure_cases.txt", by
            default False
        ...
        while not valid:
            ...
            try:
                ...
                if (
                    len(selfie_mutated_chars) > max_molecules_len
                    or smiles_canon == ""
                    or substructure_preserver(substructure, mol) is False
                ):
                    done = False
                if done:
                    valid = True
